...
const Photo = ({ resource }: photoProps) => {
  ...
  async function handleAssetDiscarding() {
    try {
      const result = await fetch("/api/discard-assets/", {
        method: "POST",
        body: JSON.stringify({
  ...
  return (
    <main className="w-full ">
      <header className="flex items-center justify-between h-16 px-4 border-b shrink-0 md:px-6">
        <a
          href="/"
          className="rounded-full hover:outline-2 p-3 shadow-sm delay-50 hover:shadow-md duration-500"
        >
          ...
          <Sheet>
            ...
            <SheetContent>
              <SheetHeader>
                ...
                <SheetDescription>
                  Transform your image with various tools.
                </SheetDescription>
              </SheetHeader>
              <div className=" gap-4 py-4 flex flex-col items-start">
                ...
                <Button
                  onClick={() => setTransform("removeBackground")}
                  variant="outline"
                  className="w-full flex justify-start"
                >
                  <TrashIcon className="h-5 w-5 mr-2" />
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction  onClick={handleAssetDiscarding}>
                  Continue
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
